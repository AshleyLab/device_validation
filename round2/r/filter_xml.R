...
subject_dir="/home/anna/device_validation/round2/subject2/LoRi/Apple/"
fname="export.xml"
...
first=as.POSIXct(strptime(as.character("20160607070000"),"%Y%m%d%H%M%S"),tz="PDT")
last=as.POSIXct(strptime(as.character( "20160810083000"),"%Y%m%d%H%M%S"),tz="PDT")
...
for(i in 1:length(apple_xml))
{
  if(is.na(apple_xml[[i]]["type"]))
    next 
  fromwatch=grepl("Watch",apple_xml[[i]][["sourceName"]])
  if(is.null(apple_xml[[i]][["sourceName"]]))
    next
  if(fromwatch==FALSE)
    next
...
write.table(apple,file=paste("apple.tsv",sep=""),sep='\t',row.names = FALSE)