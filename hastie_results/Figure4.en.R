...
type_device=sort(c("Fitbit","Microsoft","PulseOn","Apple","Basis"))
legend_vals=sort(c("Fitbit Surge","Microsoft Band","PulseOn","Apple Watch","Basis Peak"))
...
for(dev in type_device){
  subl=paste("en",type_exercise,dev,sep="_")
  datal[[dev]]=data.matrix(ashdevices[,subl])
}
exmat=do.call("rbind",datal)
N=nrow(datal[[1]])
exdemog=ashdemog[rep(seq(N),5),]
exdemog$device=factor(rep(type_device,rep(N,5)))
...
mit2=mit.colors[c(1:4,6)]
pcs=scale(exsvd$u,FALSE,1/exsvd$d);colnames(pcs)=paste("PC",1:7,sep="")

par(mfrow=c(1,1),cex.axis=2,cex.lab=2,cex.main=2,cex.sub=2)
plot(pcs[,1:2],col=mit2[as.numeric(exframe$device)],pch=19,cex=2,xlim=c(-3,2),ylim=c(-3,2))
legend("topright",col=mit2,pch=19,legend=legend_vals,cex=2)
abline(h=0,v=0,lty=2)
for(i in seq(type_device)){
  tdev=type_device[i]
  dpcs=pcs[exframe$device==tdev,c(1,2)]
  cmat=cov(dpcs)
  cent=apply(pcs[exframe$device==tdev,c(1,2)],2,mean)
  ell=ellipse(cmat,centre=cent,lwd=6)
  lines(ell,col=mit2[i],lwd=6)
}