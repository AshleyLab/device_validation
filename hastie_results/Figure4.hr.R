...
par(mfrow=c(1,1),cex.axis=2,cex.lab=2,cex.main=2,sex.sub=2)
plot(pcs[,1:2],col=mit.colors[as.numeric(exframe$device)],pch=19,cex=2)
...
for(i in 1:2){
  boxplot(split(pcs[,i],exframe$device),col=mit.colors[1:7],las=2,horizontal=T,main=paste("PC",i,sep=""),ylim=c(-.6,1.5))
  abline(v=0)
}
